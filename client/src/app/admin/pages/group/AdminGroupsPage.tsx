...
import {
  Users, Search, Filter, CheckCircle2, Clock, XCircle,
  UserCheck, Layers, ChevronRight, Loader2, AlertCircle,
  ArrowLeftRight, UserCog, RefreshCw, X, GraduationCap,
  BookOpen, BarChart3, Check,
} from "lucide-react";
...
const STATUS_CFG: Record<GroupStatus, { ar: string; fr: string; icon: React.ElementType; color: string; bg: string }> = {
  OPEN:     { ar: "مفتوح",  fr: "Ouvert",  icon: CheckCircle2, color: "#2B6F5E", bg: "rgba(43,111,94,0.10)"   },
  FULL:     { ar: "ممتلئ",  fr: "Complet", icon: XCircle,      color: "#ef4444", bg: "rgba(239,68,68,0.10)"   },
  FINISHED: { ar: "منتهي", fr: "Terminé", icon: Clock,        color: "#94a3b8", bg: "rgba(148,163,184,0.10)" },
};

const ENROLL_STATUS_CFG: Record<string, { ar: string; color: string; bg: string }> = {
...
const COURSE_FLAG: Record<string, string> = {
  ENG: "🇬🇧", FRN: "🇫🇷", ESP: "🇪🇸", ARA: "🇩🇿", DEU: "🇩🇪", default: "🌐",
};

// ─── Capacity Bar ─────────────────────────────────────────────
function CapacityBar({ pct, count, max }: { pct: number; count: number; max: number }) {
    ...
    <div className="space-y-1">
      <div className="flex justify-between text-[11px]">
        ...
        <span className="font-semibold text-[#1B1B1B] dark:text-[#E5E5E5]">
          {count}<span className="text-[#9B8E82] font-normal">/{max}</span>
        </span>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 mb-1">
            <span className={`text-[13px] font-semibold truncate ${selected ? "text-[#2B6F5E] dark:text-[#4ADE80]" : "text-[#1B1B1B] dark:text-[#E5E5E5]"}`}>
              {group.name}
            </span>
          ...
          <div className="flex items-center gap-2">
            <span className="px-1.5 py-0.5 rounded text-[10px] font-bold" style={{ background: lvl.bg, color: lvl.color }}>
          ...
          <div className="mt-1.5 h-1 rounded-full bg-[#F0EBE5] dark:bg-[#2A2A2A] overflow-hidden">
            <div className="h-full rounded-full transition-all"
              style={{ width: `${fill}%`, background: fill >= 90 ? "#ef4444" : fill >= 70 ? "#f59e0b" : "#2B6F5E" }} />
          </div>
        </div>

        <ChevronRight className={`w-4 h-4 shrink-0 transition-transform ${selected ? "text-[#2B6F5E] dark:text-[#4ADE80] rotate-90" : "text-[#C8BFB5] dark:text-[#333]"}`} />
      </div>
    </button>
  );
}
...
// ─── Change Status Modal ──────────────────────────────────────
function StatusModal({ group, onClose }: { group: Group; onClose: () => void }) {
  const { mutate, isPending } = useChangeGroupStatus();
  const statuses: GroupStatus[] = ["OPEN", "FULL", "FINISHED"];

  const handle = (status: GroupStatus) => {
    mutate({ groupId: group.group_id, status }, {
      onSuccess: () => { toast.success("تم تغيير حالة الفوج"); onClose(); },
  ...
  return (
    <Overlay onClose={onClose}>
      <div className="bg-white dark:bg-[#161616] rounded-2xl p-6 w-full max-w-sm shadow-2xl">
        <h3 className="text-base font-bold text-[#1B1B1B] dark:text-[#E5E5E5] mb-1">تغيير حالة الفوج</h3>
        ...
        <div className="space-y-2">
          {statuses.map(s => {
            ...
            return (
              <button key={s} onClick={() => !isActive && handle(s)} disabled={isPending || isActive}
                ...
                <span className="text-[13px] font-medium text-[#1B1B1B] dark:text-[#E5E5E5] flex-1 text-start">{cfg.ar}</span>
                {isActive && <Check className="w-4 h-4 text-[#2B6F5E]" />}
              </button>
            );
          })}
        </div>
        <button onClick={onClose} className="mt-4 w-full py-2 rounded-xl border border-[#E8E0D5] dark:border-[#2A2A2A] text-[13px] text-[#9B8E82] hover:bg-[#F5F0EB] dark:hover:bg-[#1A1A1A] transition-colors">
          إلغاء
        </button>
      </div>
    </Overlay>
...
function TeacherModal({ group, onClose }: { group: Group; onClose: () => void }) {
  ...
  const assign = (teacher_id: string | null) => {
    mutate({ groupId: group.group_id, teacher_id }, {
      onSuccess: () => { toast.success(teacher_id ? "تم تعيين الأستاذ" : "تم إلغاء تعيين الأستاذ"); onClose(); },
      onError:   (e: any) => toast.error(e?.response?.data?.message ?? "خطأ"),
    });
  };
  ...
  return (
    <Overlay onClose={onClose}>
      <div className="bg-white dark:bg-[#161616] rounded-2xl p-6 w-full max-w-md shadow-2xl">
        <h3 className="text-base font-bold text-[#1B1B1B] dark:text-[#E5E5E5] mb-1">تعيين أستاذ</h3>
        ...
        <div className="relative mb-3">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="بحث عن أستاذ..."
        ...
        <div className="max-h-60 overflow-y-auto space-y-1 mb-3">
          {isLoading ? (
            <div className="py-8 flex justify-center"><Loader2 className="w-5 h-5 animate-spin text-[#9B8E82]" /></div>
          ) : filtered.map((t: any) => {
            const isCurrent = group.teacher?.teacher_id === t.teacher_id;
            return (
              <button key={t.teacher_id} onClick={() => assign(t.teacher_id)} disabled={isPending}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all ${
                  isCurrent ? "bg-[#EDF6F3] dark:bg-[#0F2420]" : "hover:bg-[#F5F0EB] dark:hover:bg-[#111]"
                }`}>
                ...
                <div className="flex-1 text-start">
                  <p className="text-[13px] font-medium text-[#1B1B1B] dark:text-[#E5E5E5]">{t.first_name} {t.last_name}</p>
          ...
          {!isLoading && filtered.length === 0 && (
            <p className="py-6 text-center text-[12px] text-[#9B8E82]">لا توجد نتائج</p>
          )}
        </div>
        ...
        <button onClick={onClose} className="w-full py-2 rounded-xl border border-[#E8E0D5] dark:border-[#2A2A2A] text-[13px] text-[#9B8E82] hover:bg-[#F5F0EB] dark:hover:bg-[#1A1A1A] transition-colors">
          إلغاء
        </button>
      </div>
    </Overlay>
...
// ─── Transfer Modal ───────────────────────────────────────────
function TransferModal({ student, fromGroup, allGroups, onClose }: {
  ...
  const handle = () => {
    ...
    mutate({
      ...
      onSuccess: () => { toast.success(`تم نقل ${student.student.first_name} إلى ${selectedGroup.name}`); onClose(); },
      onError:   (e: any) => toast.error(e?.response?.data?.message ?? "خطأ في النقل"),
    });
  };
  ...
  return (
    <Overlay onClose={onClose}>
      <div className="bg-white dark:bg-[#161616] rounded-2xl p-6 w-full max-w-md shadow-2xl">
        <h3 className="text-base font-bold text-[#1B1B1B] dark:text-[#E5E5E5] mb-1">نقل الطالب</h3>
        <p className="text-[12px] text-[#9B8E82] mb-4">
          {student.student.first_name} {student.student.last_name} — من: <span className="font-semibold">{fromGroup.name}</span>
        </p>

        <div className="relative mb-3">
          <Search className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#9B8E82]" />
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="ابحث عن فوج..."
        ...
        <div className="max-h-52 overflow-y-auto space-y-1 mb-4">
          {eligible.length === 0 ? (
            <p className="py-8 text-center text-[12px] text-[#9B8E82]">لا توجد أفواج متاحة لنفس المادة</p>
          ) : eligible.map(g => (
            <button key={g.group_id} onClick={() => setSelectedGroup(g)}
              ...
              <div className="flex-1 text-start">
                <p className="text-[13px] font-medium text-[#1B1B1B] dark:text-[#E5E5E5]">{g.name}</p>
                <p className="text-[11px] text-[#9B8E82]">{g.level.replace("_","-")} · {g.enrolled_count}/{g.max_students}</p>
              </div>
              <StatusBadge status={g.status} />
              {selectedGroup?.group_id === g.group_id && <Check className="w-4 h-4 text-[#2B6F5E] shrink-0" />}
            </button>
          ))}
        </div>

        <div className="flex gap-2">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border border-[#E8E0D5] dark:border-[#2A2A2A] text-[13px] text-[#9B8E82]">
            إلغاء
          </button>
          <button onClick={handle} disabled={!selectedGroup || isPending}
...
function GroupDetails({ group, allGroups }: { group: Group; allGroups: Group[] }) {
  const [statusModal, setStatusModal] = useState(false);
  const [teacherModal, setTeacherModal] = useState(false);
  const [transferStudent, setTransferStudent] = useState<GroupStudent | null>(null);
  const [enrollFilter, setEnrollFilter] = useState<string>("ALL");

  const { data: studentsData, isLoading: loadingStudents, refetch } =
    useAdminGroupStudents(group.group_id);

  const students = studentsData?.data ?? [];

  const filtered = useMemo(() =>
    enrollFilter === "ALL" ? students : students.filter(s => s.registration_status === enrollFilter),
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-5 border-b border-[#F0EBE5] dark:border-[#1E1E1E] shrink-0">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-[16px] font-bold text-[#1B1B1B] dark:text-[#E5E5E5] leading-tight">{group.name}</h2>
            <p className="text-[12px] text-[#9B8E82]">{group.course.course_name}</p>
            <div className="flex items-center gap-2 mt-1.5 flex-wrap">
              <span className="px-2 py-0.5 rounded text-[10px] font-bold" style={{ background: lvl.bg, color: lvl.color }}>
                {group.level.replace("_","-")}
              </span>
              <StatusBadge status={group.status} />
            </div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          {[
            { icon: Users,       label: "الطلبة",    value: `${group.enrolled_count}/${group.max_students}` },
            { icon: Layers,      label: "الحصص",     value: group._count.sessions },
            { icon: BarChart3,   label: "الإشغال",   value: `${group.capacity_pct}%` },
          ].map(({ icon: Icon, label, value }) => (
            <div key={label} className="bg-[#F5F0EB] dark:bg-[#111] rounded-xl p-3 text-center">
              <Icon className="w-4 h-4 text-[#2B6F5E] dark:text-[#4ADE80] mx-auto mb-1" />
              <p className="text-[14px] font-bold text-[#1B1B1B] dark:text-[#E5E5E5]">{value}</p>
              <p className="text-[10px] text-[#9B8E82]">{label}</p>
            </div>
          ))}
        </div>

        <CapacityBar pct={group.capacity_pct} count={group.enrolled_count} max={group.max_students} />
        ...
        <div className="flex items-center gap-2 mt-3 p-3 rounded-xl bg-[#F5F0EB] dark:bg-[#111]">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] text-[#9B8E82] uppercase tracking-wide">الأستاذ</p>
            <p className={`text-[13px] font-medium truncate ${group.teacher ? "text-[#1B1B1B] dark:text-[#E5E5E5]" : "text-[#9B8E82] italic"}`}>
              {group.teacher ? `${group.teacher.first_name} ${group.teacher.last_name}` : "غير محدد"}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {loadingStudents ? (
            ...
            <div className="divide-y divide-[#F5F0EB] dark:divide-[#1A1A1A]">
              {filtered.map(s => {
                const cfg = ENROLL_STATUS_CFG[s.registration_status] ?? ENROLL_STATUS_CFG.PENDING;
                const canTransfer = s.registration_status === "VALIDATED" || s.registration_status === "PAID";
                return (
                  <div key={s.enrollment_id} className="flex items-center gap-3 px-4 py-3 hover:bg-[#FDFAF7] dark:hover:bg-[#0D0D0D] group/row transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-[#9B8E82] truncate">{s.student.email}</p>
                    </div>
      ...
      {/* Modals */}
      {statusModal  && <StatusModal  group={group} onClose={() => setStatusModal(false)} />}
      {teacherModal && <TeacherModal group={group} onClose={() => setTeacherModal(false)} />}
      {transferStudent && (
        <TransferModal
...
export default function AdminGroupsPage() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] bg-[#FDFAF7] dark:bg-[#0A0A0A]" dir="rtl">

      {/* ── Top bar ── */}
      <div className="px-5 py-4 bg-white dark:bg-[#0D0D0D] border-b border-[#E8E0D5] dark:border-[#1E1E1E] shrink-0">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-[18px] font-bold text-[#1B1B1B] dark:text-[#E5E5E5]">إدارة الأفواج</h1>
            <p className="text-[12px] text-[#9B8E82]">{stats.total} فوج — {stats.open} مفتوح · {stats.full} ممتلئ · {stats.finished} منتهي</p>
          </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* LEFT — Groups list */}
        <div className={`shrink-0 border-l border-[#E8E0D5] dark:border-[#1E1E1E] flex flex-col bg-white dark:bg-[#0D0D0D] overflow-hidden transition-all duration-300 ${
          ...
          <div className="px-4 py-2.5 border-b border-[#F0EBE5] dark:border-[#1A1A1A] flex items-center justify-between shrink-0">
            <span className="text-[12px] text-[#9B8E82]">{groups.length} فوج</span>
            {selected && (
              <button onClick={() => setSelectedId(null)} className="w-6 h-6 rounded-lg flex items-center justify-center text-[#9B8E82] hover:text-[#1B1B1B] dark:hover:text-[#E5E5E5] hover:bg-[#F0EBE5] dark:hover:bg-[#1A1A1A] transition-colors">
          ...
          <div className="flex-1 overflow-y-auto">
            {isLoading ? (
              <div className="py-20 flex justify-center"><Loader2 className="w-6 h-6 animate-spin text-[#2B6F5E]" /></div>
            ) : isError ? (
              <div className="py-20 text-center">
                <AlertCircle className="w-10 h-10 text-red-400 mx-auto mb-3" />
                <p className="text-[13px] text-[#9B8E82] mb-3">خطأ في تحميل البيانات</p>
                <button onClick={() => refetch()} className="px-4 py-2 rounded-xl bg-[#2B6F5E] text-white text-[12px]">إعادة المحاولة</button>
              </div>
            ) : groups.length === 0 ? (
              <div className="py-20 text-center">
        ...
        {selected ? (
          ...
          <div className="hidden lg:flex flex-1 items-center justify-center bg-[#FDFAF7] dark:bg-[#0A0A0A]">
            <div className="text-center">
              ...
              <p className="text-[14px] font-medium text-[#9B8E82]">اختر فوجاً لعرض التفاصيل</p>
              <p className="text-[12px] text-[#C8BFB5] dark:text-[#444] mt-1">اضغط على أي فوج من القائمة</p>
            </div>
          </div>
        )}
      </div>
