    ...
    >
      ...
      <div className="relative h-44 overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 z-10">
          <p className="text-white text-xl font-bold capitalize">{lang}</p>
...
export default function CoursesHomePage() {
  ...
  courses.forEach((c) => {
    const key = c.language?.toLowerCase() || "other";
    if (!langMap[key]) langMap[key] = [];
    langMap[key].push(c);
  });
  ...
  const breadcrumbSteps = [
    ...
    ...(selectedLang
      ? [
          {
            label: selectedLang.charAt(0).toUpperCase() + selectedLang.slice(1),
            onClick:
