      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 -mt-4 pb-16">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 space-y-6">
            ...
            {course.level && (
              <div
                className="bg-white rounded-2xl border border-brand-beige p-6 shadow-sm animate-fade-up"
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs text-brand-brown font-medium uppercase tracking-wider">
                      Level
                    </p>
            ...
            {course.pricing && course.pricing.length > 0 && (
              <div
                className="bg-white rounded-2xl border border-brand-beige overflow-hidden shadow-sm animate-fade-up"
                ...
                <div className="bg-gradient-to-r from-brand-teal-dark to-brand-teal px-6 py-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white/60 text-xs">
                      Fees vary based on applicant category
                    </p>
                  </div>
                </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-brand-gray/50">
                        <th className="text-left px-6 py-3 font-semibold text-brand-brown text-xs uppercase tracking-wider">
                          Status
                        </th>
                        <th className="text-left px-5 py-3 font-semibold text-brand-brown text-xs uppercase tracking-wider">
                          Details
                        </th>
                        <th className="text-right px-6 py-3 font-semibold text-brand-brown text-xs uppercase tracking-wider">
                          Price
                        </th>
                        <th className="text-center px-5 py-3 font-semibold text-brand-brown text-xs uppercase tracking-wider">
                          Discount
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {course.pricing.map((p: any, i: number) => (
                        <tr
                          key={p.id || i}
                          className={`border-t border-brand-beige/60 transition-colors hover:bg-brand-teal-dark/[0.02] ${
                            i % 2 !== 0 ? "bg-brand-gray/30" : ""
                          }`}
                        >
                          ...
                          <td className="px-5 py-4">
                            <div className="space-y-0.5">
                              {p.status_ar && (
                                <p
                                  className="text-brand-black/55 text-sm"
                                  dir="rtl"
                                >
                                  {p.status_ar}
                                </p>
                          ...
                          <td className="px-6 py-4 text-right">
                            <span className="font-bold text-brand-black tabular-nums text-base">
                          ...
                          <td className="px-5 py-4 text-center">
                            <span
                              className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold ${
                                !p.discount || p.discount === "Aucune" || p.discount === "None"
                                  ? "bg-brand-beige/80 text-brand-brown"
              ...
              >
                <div className="flex items-center gap-3 px-1">
                  ...
                  <div>
                    ...
                    <p className="text-brand-brown text-xs">
                      {course.groups.length} group
                      {course.groups.length > 1 ? "s" : ""} available
                    </p>
                  </div>
                </div>
              ...
              <div className="p-5 space-y-4">
                ...
                {course.price !== null && Number(course.price) > 0 && (
                  <div className="pt-3.5 border-t border-brand-beige/60">
                    <p className="text-[11px] text-brand-brown uppercase tracking-wider font-medium">
                      Starting from
                    </p>
                    <p className="text-2xl font-bold text-brand-teal-dark mt-0.5">
                ...
                <div className="space-y-2.5 pt-1">
                  {isOpen && (
                    <Button
                      asChild
                      className="w-full bg-brand-teal-dark hover:bg-brand-teal-dark/90 text-white gap-2 h-12 text-sm font-semibold rounded-xl shadow-lg shadow-brand-teal-dark/20"
                    >
            ...
            <div className="bg-white rounded-2xl border border-brand-beige p-5 shadow-sm">
              ...
              <div className="space-y-3">
                ...
                <QuickRow
                  ...
                  label="Groups"
                  value={`${course.groups?.length || 0} available`}
                />
                {course.fee_amount && Number(course.fee_amount) > 0 && (
                  <QuickRow
                    icon={<TrendingUp className="w-3.5 h-3.5" />}
                    label="Admin Fee"
                    value={`${Number(course.fee_amount).toLocaleString()} DA`}
                  />
                )}
              </div>
            </div>
...
function InfoChip({
  ...
  return (
    <div
      className={`rounded-xl border p-4 text-center ${styles[color]}`}
    >
      <div
        className={`mx-auto mb-2 ${iconStyles[color]}`}
      >
        {icon}
      </div>
      <p className="text-[10px] text-brand-brown uppercase tracking-wider font-medium">
        {label}
      </p>
...
function QuickRow({
  ...
  return (
    <div className="flex items-center gap-2.5 text-sm">
      ...
      <span className="ml-auto font-semibold text-brand-black truncate">
        {value}
      </span>
    </div>
  );
