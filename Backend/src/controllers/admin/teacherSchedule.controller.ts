...
const DAYS_AR: Record<number, string> = {
  ...
  3: "الثلاثاء", 4: "الأربعاء", 5: "الخميس",
};
...
// ══════════════════════════════════════════════════════════════
export async function getAllTeacherScheduleController(req: Request, res: Response) {
  try {
    const { teacher_id, day_of_week, language } = req.query as Record<string, string>;
    ...
    const entries = await prisma.teacherScheduleEntry.findMany({
      where,
      include: INCLUDE,
      orderBy: [{ teacher_id: "asc" }, { day_of_week: "asc" }, { start_time: "asc" }],
    });
...
//  جدول أستاذ كامل أسبوعياً
// ══════════════════════════════════════════════════════════════
export async function getTeacherScheduleController(req: Request, res: Response) {
  try {
    ...
    const [teacher, entries] = await Promise.all([
      prisma.teacher.findUnique({
        ...
        select: {
          ...
          last_name:  true,
          email:      true,
          user: { select: { google_avatar: true, email: true, is_active: true } },
          groups: {
            select: {
              group_id: true, name: true, level: true, status: true,
    ...
    if (!teacher) {
      return res.status(404).json({ success: false, message: "الأستاذ غير موجود" });
    }

    // تجميع حسب اليوم
    const weekly: Record<number, any[]> = { 0: [], 1: [], 2: [], 3: [], 4: [], 5: [] };
    for (const e of entries) weekly[e.day_of_week].push(e);

    return res.json({ success: true, teacher, weekly, entries, total: entries.length });
  } catch (error) {
...
// ══════════════════════════════════════════════════════════════
//  GET /admin/teacher-schedule/teacher/:teacherId/view
//  للأستاذ نفسه — يرى جدوله فقط (بدون بيانات حساسة)
// ══════════════════════════════════════════════════════════════
  ...
  try {
    ...
    const entries = await prisma.teacherScheduleEntry.findMany({
      ...
      include: {
        group: {
          select: {
            group_id: true, name: true, level: true,
            course:   { select: { course_name: true } },
          },
        },
...
//  Body: { teacher_id, group_id?, room_id?, day_of_week,
//          start_time, end_time, language, level, group_label, notes? }
// ══════════════════════════════════════════════════════════════
export async function createTeacherScheduleController(req: Request, res: Response) {
  try {
    const {
      teacher_id, group_id, room_id,
      day_of_week, start_time, end_time,
      language, level, group_label, notes,
    } = req.body;
    ...
    const { conflict, existing } = await checkTeacherConflict(
      teacher_id, Number(day_of_week), start_time, end_time,
    );
    if (conflict) {
    ...
    if (error.code === "P2002") {
      return res.status(409).json({ success: false, message: "تعارض: الأستاذ مسجّل في هذا الوقت" });
    }
...
// ══════════════════════════════════════════════════════════════
export async function updateTeacherScheduleController(req: Request, res: Response) {
  try {
    const { entryId } = req.params;

    const existing = await prisma.teacherScheduleEntry.findUnique({ where: { entry_id: entryId } });
    if (!existing) {
    ...
    const checkDay   = day_of_week  !== undefined ? Number(day_of_week) : existing.day_of_week;
    ...
    const { conflict, existing: conflictWith } = await checkTeacherConflict(
      existing.teacher_id, checkDay, checkStart, checkEnd, entryId,
    );
    if (conflict) {
      return res.status(409).json({
        success: false,
        message: "تعارض في التوقيت بعد التعديل",
        conflict: { day: DAYS_AR[conflictWith.day_of_week], slot: `${conflictWith.start_time} - ${conflictWith.end_time}` },
      });
    }
    ...
    const updated = await prisma.teacherScheduleEntry.update({
      ...
      data: {
        ...
        ...(rest.group_label !== undefined && { group_label: rest.group_label.trim() }),
        ...(rest.notes       !== undefined && { notes:       rest.notes }),
      },
      include: INCLUDE,
    });

    return res.json({ success: true, message: "تم تحديث الحصة", data: updated });
  } catch (error: any) {
    if (error.code === "P2002") {
      return res.status(409).json({ success: false, message: "تعارض في التوقيت" });
    }
...
// ══════════════════════════════════════════════════════════════
export async function deleteTeacherScheduleController(req: Request, res: Response) {
  try {
    const { entryId } = req.params;

    const entry = await prisma.teacherScheduleEntry.findUnique({ where: { entry_id: entryId } });
    if (!entry) {
      return res.status(404).json({ success: false, message: "الحصة غير موجودة" });
    }
...
//  حذف كل جدول أستاذ
// ══════════════════════════════════════════════════════════════
export async function clearTeacherScheduleController(req: Request, res: Response) {
  try {
    const { teacherId } = req.params;
    ...
    return res.json({ success: true, message: `تم حذف ${count} حصة`, deleted: count });
  } catch (error) {
