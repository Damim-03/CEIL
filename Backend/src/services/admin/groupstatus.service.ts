...
// ✅ Same { error, data } pattern as course.service.ts
// ================================================================
...
function computeCapacity(
  ...
  const active  = enrollments.filter(e => ["VALIDATED", "PAID"].includes(e.registration_status)).length;
  const pending = enrollments.filter(e => e.registration_status === "PENDING").length;
  return {
    enrolled_count: active,
    pending_count:  pending,
    capacity_pct:   Math.round((active / max) * 100),
    is_full:        active >= max,
  };
}

// ─── Socket emit helpers (fire-and-forget) ────────────────────
// Using same room structure from socket.ts:
//   role:ADMIN_LEVEL  → ADMIN + OWNER
//   group:{id}        → anyone watching this group
//   user:{id}         → specific user (student)
...
function emitGroupStatusChanged(group_id: string, name: string, status: GroupStatus) {
  try {
    getIO()
      .to("role:ADMIN_LEVEL")
      .emit("group:statusChanged", {
        group_id,
        name,
        status,
        timestamp: new Date().toISOString(),
...
function emitStudentTransferred(payload: {
  ...
  try {
    getIO()
      ...
      .to(`user:${payload.student_id}`)   // notify the student too
...
export async function getGroups(params: {
  ...
  const { status, level, course_id, teacher_id, search, page = 1, limit = 20 } = params;
  ...
  const [groups, total] = await Promise.all([
    prisma.group.findMany({
      ...
      select: {
        ...groupBaseSelect,
        enrollments: {
          where:  { registration_status: { in: ["VALIDATED", "PAID", "PENDING"] } },
...
export async function changeGroupStatus(
  ...
  const updated = await prisma.$transaction(async tx => {
    ...
    // FINISHED → close all active enrollments automatically
    if (status === "FINISHED") {
...
export async function transferStudent(input: {
  ...
  const [fromGroup, toGroup] = await Promise.all([
    prisma.group.findUnique({
      ...
      select: { group_id: true, name: true, status: true, max_students: true, course_id: true },
    }),
    prisma.group.findUnique({
  ...
  const targetCount = await prisma.enrollment.count({
    where: { group_id: to_group_id, registration_status: { in: ["VALIDATED", "PAID"] } },
  });
  if (targetCount >= toGroup.max_students) return { error: "target_full" as const };
  ...
  const result = await prisma.$transaction(async tx => {
    ...
    // Auto-update group statuses based on new capacity
    const [newFromCount, newToCount] = await Promise.all([
      tx.enrollment.count({ where: { group_id: from_group_id, registration_status: { in: ["VALIDATED","PAID"] } } }),
      tx.enrollment.count({ where: { group_id: to_group_id,   registration_status: { in: ["VALIDATED","PAID"] } } }),
    ]);

    const statusUpdates: Promise<any>[] = [];
    if (fromGroup.status === "FULL" && newFromCount < fromGroup.max_students)
      statusUpdates.push(tx.group.update({ where: { group_id: from_group_id }, data: { status: "OPEN" } }));
    if (newToCount >= toGroup.max_students)
      statusUpdates.push(tx.group.update({ where: { group_id: to_group_id },   data: { status: "FULL" } }));
    if (statusUpdates.length) await Promise.all(statusUpdates);

    return updated;
  });

  // 🔴 Realtime — notify admins, both group rooms, and the student
...
export async function assignTeacher(group_id: string, teacher_id: string | null) {
  const group = await prisma.group.findUnique({
    where: { group_id }, select: { group_id: true },
  });
  if (!group) return { error: "not_found" as const };
